...
async def create_order(order_data: OrderCreate, current_user: User = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid Razorpay credentials. Please configure valid API keys in backend .env file."
        )
    except Exception as e:
        logging.error(f"Error creating order: {e}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Failed to create payment order. Please check Razorpay credentials. Error: {str(e)}"
        )
